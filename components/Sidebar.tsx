...
const Sidebar: React.FC<SidebarProps> = ({ role, activeTab, setActiveTab, requests, isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[49] lg:hidden"
          onClick={() => setIsOpen(false)}
        />
      )}

      <aside 
        className={`fixed top-0 right-0 h-screen w-80 bg-[#0f0f13] border-l border-white/5 p-8 flex flex-col z-[50] transition-transform duration-300 ease-in-out lg:static lg:transform-none ${
          isOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'
        }`}
      >
        <div className="flex flex-col h-full">
          <div className="mb-14 px-2 flex justify-between items-start">
            <div>
